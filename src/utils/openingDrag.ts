...
export function predictOpeningDrag(raw: PlacedItem, walls: Wall[], items: PlacedItem[], zoom: number, previousWallId?: string, snapDistancePx = 56, allowFree = false): OpeningDragPreview {
  const candidates = walls.flatMap(wall => {
    ...
    return distance <= snapDistancePx ? [{ wall, placement, distance }] : [];
  ...
  if (!target) {
    const free = allowFree || (!raw.wallId && walls.length === 0);
