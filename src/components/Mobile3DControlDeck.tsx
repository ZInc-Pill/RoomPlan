...
interface Mobile3DControlDeckProps {
  ...
  onOpenCatalog: () => void;
  onOpenLayers: () => void;
  onCameraPreset: (preset: 'perspective' | 'walk' | 'isometric') => void;
  onFocusSelection: () => void;
  currentCameraPreset: 'perspective' | 'walk' | 'isometric';
  onSnapshot?: () => void;
}
...
export function Mobile3DControlDeck(props: Mobile3DControlDeckProps) {
  ...
  if (props.isPanelOpen || props.currentCameraPreset === 'walk') return null;
  ...
  return (
    <section aria-label="3D controls" data-editor-control className="fixed bottom-20 inset-x-0 z-30 md:hidden px-3 pb-[calc(0.5rem+env(safe-area-inset-bottom,0px))] pointer-events-none">
      <div className="mx-auto max-w-md rounded-2xl border border-slate-200 bg-white/95 p-3 shadow-xl backdrop-blur pointer-events-auto max-h-[45dvh] overflow-y-auto overscroll-contain" onPointerDown={e => e.stopPropagation()} onWheel={e => e.stopPropagation()}>
        ...
        <p className="text-xs text-slate-500 mb-3">One finger rotates the view. Two fingers move it. Pinch to zoom. Tap an object to select it.</p>
        {selected && <div className="grid grid-cols-4 gap-1 mb-3" aria-label="Editing tools">
          {(['move', 'rotate', ...(props.selectedItem ? ['height'] : [])] as const).map(value => <button key={value} aria-pressed={tool === value} className={button + (tool === value ? ' !bg-indigo-600 !text-white !border-indigo-600' : '')} onClick={() => setTool(value as typeof tool)}>{value[0].toUpperCase() + value.slice(1)}</button>)}
        </div>}
        {selected && tool === 'move' && <>
          <div className="flex gap-2 items-center mb-2"><label className="text-xs flex-1" htmlFor="move-step">Move step</label><select id="move-step" value={stepCm} onChange={e => setStepCm(Number(e.target.value))} className={button}>{[1, 5, 10, 25, 50].map(value => <option key={value} value={value}>{value} cm</option>)}</select></div>
